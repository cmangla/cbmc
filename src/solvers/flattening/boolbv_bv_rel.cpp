...
literalt boolbvt::convert_bv_rel(const binary_relation_exprt &expr)
{
  ...
  if(bv0.size() == bv1.size() && !bv0.empty() && bvtype0 == bvtype1)
  {
    if(bvtype0 == bvtypet::IS_FLOAT)
    {
      ...
      (op0.type().id() == ID_range && op1.type() == op0.type()) ||
      bvtype0 == bvtypet::IS_SIGNED || bvtype0 == bvtypet::IS_UNSIGNED ||
      bvtype0 == bvtypet::IS_FIXED)
    {
      ...
      (bvtype0 == bvtypet::IS_VERILOG_SIGNED ||
